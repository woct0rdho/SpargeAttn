...
The latest wheels support GTX 16xx, RTX 20xx/30xx/40xx/50xx, A100, H100, AGX Orin (sm75/80/86/87/89/90/120). There are also reports that SpargeAttention works with B200 (sm100) and DGX Spark (sm121), but I did not bundle these kernels in the wheels, and you need to build from source.
...
* For Turing GPUs (GTX 16xx, RTX 20xx), you need to use the Triton kernel `spas_sage_attn_meansim` in `spas_sage_attn/triton_kernel_example.py`
